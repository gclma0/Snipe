...
from app.ai.profile_interpreter import interpret_profile
...
def test_profile_interpreter_detects_non_technical_specialization_and_seniority() -> None:
    result = interpret_profile(normalized_profile())

    assert result.primary_specialization is not None
    assert result.primary_specialization.name == "operations"
    assert result.estimated_seniority == "senior"


def test_readiness_dashboard_aggregates_scores_with_skill_alignment() -> None:
    result = build_readiness_dashboard(normalized_profile(), structured_job())

    assert result.scores.overall >= 80
    assert result.scores.skill_alignment is not None
    assert result.skill_gap is not None
    assert result.checks["has_target_job"] is True


def test_readiness_dashboard_endpoint_persists_analysis() -> None:
    ...
    response = client.post(
        f"/api/v1/profiles/{TEST_PROFILE_ID}/analyses/readiness-dashboard",
        json={"job_description_id": TEST_JOB_ID},
    )

    assert response.status_code == 201
    body = response.json()
    assert body["analysis_type"] == "readiness_dashboard"
    assert fake.analyses[0]["analysis_type"] == "readiness_dashboard"
    assert fake.analyses[0]["score"] == body["scores"]["overall"]


def test_readiness_dashboard_endpoint_requires_normalized_profile() -> None:
    fake = FakeSupabaseClient(
        profile={
            "id": TEST_PROFILE_ID,
            "user_id": TEST_USER_ID,
    ...
    response = client.post(f"/api/v1/profiles/{TEST_PROFILE_ID}/analyses/readiness-dashboard")
